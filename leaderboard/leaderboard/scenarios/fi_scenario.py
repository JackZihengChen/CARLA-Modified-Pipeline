...
class FIScenario(BasicScenario):
    ...
    def __init__(self, world, config, debug_mode=0, criteria_enable=True, arg_weather=None):
        """
        ...
        self.arg_weather = arg_weather

        if not self.arg_weather:
            raise Exception
        
        self._update_route(world, config, debug_mode>2)

        ego_vehicle = self._update_ego_vehicle(arg_weather=self.arg_weather)
    ...
    def _update_ego_vehicle(self, arg_weather=None):
        """
        Set/Update the start position of the ego_vehicle
        """
        
        if len(arg_weather) != 9:
            raise ValueError("weather parameters must have a length of 8, see help")
        cloudiness = arg_weather[0]
        precipitation = arg_weather[1]
        precipitation_deposits = arg_weather[2]
        sun_altitude_angle = arg_weather[3]
        sun_azimuth_angle = arg_weather[4]
        wetness = arg_weather[5]
        fog_density = arg_weather[6]
        wind_intensity = arg_weather[7]
        ice_thickness = arg_weather[8]
        ...
        with open('weather.txt', 'w+') as f:
            f.write(str(arg_weather) + " ")
            f.write(str(precipitation) + " ")
            f.write(str(precipitation_deposits) + " ")         
            f.write(str(wetness) + " ")
            f.write('\n')
        ego_vehicle = CarlaDataProvider.request_new_actor('vehicle.lincoln.mkz2017',
                                                          elevate_transform,
                                                          rolename='hero', 
                                                          weather_params = arg_weather, color='255,0,0')

        spectator = CarlaDataProvider.get_world().get_spectator()
        ego_trans = ego_vehicle.get_transform()
        # spectator.set_transform(carla.Transform(ego_trans.location + carla.Location(z=150),
        #                                             carla.Rotation(pitch=-90)))
        # spectator.set_transform(carla.Transform(ego_trans.location + carla.Location(z=300),
        #                                             carla.Rotation(pitch=-90)))

        
        # 192.09947204589844" y="-85.663352012634277"

        # Find Trigger Friction Blueprint
        # friction_bp = carla_world.get_blueprint_library().find('static.trigger.friction')

        extent = carla.Location(192.09, -85.66, 5)
        ...
        transform.location = carla.Location(192.09, -85.66, 0.0)
        # world.spawn_actor(friction_bp, transform)

        # # Optional for visualizing trigger
        # carla_world.debug.draw_box(box=carla.BoundingBox(transform.location, extent * 1e-2), rotation=transform.rotation, life_time=100, thickness=0.5, color=carla.Color(r=255,g=0,b=0))
    ...
    def _build_scenario_instances(self, world, ego_vehicle, scenario_definitions,
                                  scenarios_per_tick=5, timeout=300, debug_mode=False, arg_weather={}):
        """
        ...
        for scenario_number, definition in enumerate(scenario_definitions):
            ...
            try:
                scenario_instance = scenario_class(world, [ego_vehicle], scenario_configuration,
                                                   criteria_enable=False, timeout=timeout, weather_params=arg_weather)
