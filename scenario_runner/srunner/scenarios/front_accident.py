...
class FrontAccident(BasicScenario):
    ...
    def _initialize_actors(self, config):
        ...
        first_vehicle = CarlaDataProvider.request_new_actor('vehicle.audi.a2',
                                                            ...
                                                            weather_params = self.weather_params, color='0,0,255')
        ...
        second_vehicle = CarlaDataProvider.request_new_actor('vehicle.tesla.model3',
                                                            ...
                                                            weather_params = self.weather_params, color='130,130,130')
    ...
    def _create_behavior(self):
        ...
        driving_to_next_intersection_third.add_child(KeepVelocity(self.other_actors[0], 0, duration=4))
        driving_to_next_intersection_third.add_child(ChangeAutoPilot(self.other_actors[0], True,
                                                                     parameters={"max_speed": 9}))
        driving_to_next_intersection_third.add_child(LaneChange(self.other_actors[0],
                                                                 direction="right",
                                                                 distance_same_lane=0, # if you use 2 or 3 here will generate a different accident
                                                                 distance_other_lane=100,
                                                                 distance_lane_change=5,
                                                                 speed=100))
        driving_to_next_intersection_third.add_child(KeepVelocity(self.other_actors[0], 100))
        ...
        driving_to_next_intersection_forth.add_child(KeepVelocity(self.other_actors[1], 0, duration=2))
        driving_to_next_intersection_forth.add_child(ChangeAutoPilot(self.other_actors[1], True, 
                                                                     parameters={"max_speed": self._second_vehicle_speed}))
        driving_to_next_intersection_forth.add_child(KeepVelocity(self.other_actors[1], self._second_vehicle_speed))


        # get the successful state
        condition_success = StandStill(self.ego_vehicles[0], name="Ego Stand Still", duration=30)
