...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def request_new_actor(model, spawn_point, rolename='scenario', autopilot=False,
        ...
        # New
        if weather_params:
            if len(weather_params) != 9:
                raise ValueError("weather parameters must have a length of 9, see help")
            cloudiness = weather_params[0]
            precipitation = weather_params[1]
            precipitation_deposits = weather_params[2]
            sun_altitude_angle = weather_params[3]
            sun_azimuth_angle = weather_params[4]
            wetness = weather_params[5]
            fog_density = weather_params[6]
            wind_intensity = weather_params[7]
            ice_thickness = weather_params[8]

            # if ice_thickness == 0 and precipitation_deposits == 0 and wetness == 0:
            #     return actor
            
            with open('friction.txt', 'a') as f:
                old_friction = 0
                new_friction_ratio = 0

                old_wheels = actor.get_physics_control().wheels
                wheels = []
                for wheel in old_wheels:
                    old_friction = wheel.tire_friction
                    new_friction_ratio = 1
                    if ice_thickness != 0:
                        new_friction_ratio = math.exp(-1.89711*ice_thickness/100) * (1-ice_thickness/100)**3 * 0.85 + 0.15
                    elif precipitation_deposits != 0  or wetness != 0:
                        new_friction_ratio = math.exp(-0.916*wetness/100) * (1-wetness/100)**3 * 0.6 + 0.4 - 0.1*precipitation_deposits/100

                    new_friction = old_friction
                    # new_friction = old_friction * new_friction_ratio
                    # new_friction = 0.5
                    
                    curr_wheel = carla.WheelPhysicsControl(tire_friction=new_friction)
                    wheels.append(curr_wheel)
                    print(old_friction, new_friction)

                f.write("wetness: " + str(wetness) + " ")     
                f.write("precipitation_deposits: " + str(precipitation_deposits) + " ")
                f.write("ice_thickness: " + str(ice_thickness) + " ")
                f.write("\n")
                    
                f.write("old_friction: " + str(old_friction) + " ")     
                f.write("new_friction: " + str(new_friction) + " ")
                f.write("new_friction_ratio : " + str(new_friction_ratio) + " ")
                f.write("\n")
                      
        
        # old
        # front_left_wheel  = carla.WheelPhysicsControl(tire_friction=0.5, damping_rate=1.5)
        # front_right_wheel = carla.WheelPhysicsControl(tire_friction=0.5, damping_rate=1.5)
        # rear_left_wheel   = carla.WheelPhysicsControl(tire_friction=0.5, damping_rate=1.5)
        # rear_right_wheel  = carla.WheelPhysicsControl(tire_friction=0.5, damping_rate=1.5)
